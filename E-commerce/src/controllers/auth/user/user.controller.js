...
module.exports.registerUser = async (req, res) => {
    try {
        ...
        if (!req.body.email || !req.body.password) {
            return res.status(statusCode.BAD_REQUEST).json(errorResponse(statusCode.BAD_REQUEST, true, MSG.VALIDATION_ERROR));
        }
...
module.exports.fetchAllUser = async (req, res) => {
    try {
        // Yahan se "if (req.user)" wala block hata diya hai
...
module.exports.deleteUser = async (req, res) => {
    try {
        ...
        const user = await userAuthService.fetchSingleUser({ _id: id, isDelete: false }, true);

        if (!user) {
            return res.status(statusCode.BAD_REQUEST).json(errorResponse(statusCode.BAD_REQUEST, true, MSG.USER_NOT_FOUND));
        }

        const deletedUser = await userAuthService.updateUser(id, { isDelete: true, isActive: false });
...
module.exports.updateUser = async (req, res) => {
    try {
        const id = req.query.id; // Postman mein ?id=... bhejna
        req.body.update_at = moment().format('DD/MM/YYYY, h:mm:ss A');
        
        const updatedUser = await userAuthService.updateUser(id, req.body);
...
module.exports.activeOrInActiveUser = async (req, res) => {
    try {
        ...
        const updatedUser = await userAuthService.updateUser(id, { 
            isActive: !user.isActive, 
            update_at: moment().format('DD/MM/YYYY, h:mm:ss A') 
        });

        return res.status(statusCode.OK).json(successResponse(statusCode.OK, false, `${user.first_name} is now ${updatedUser.isActive ? 'Active' : 'Inactive'}`));
    } catch (err) {
        console.log("Error : ", err);
        return res.status(statusCode.INTERNAL_SERVER_ERROR).json(errorResponse(statusCode.INTERNAL_SERVER_ERROR, true, MSG.SERVER_ERROR));
    }
