    ...
    try {
        ...
        if (!req.file) {
            return res.status(statusCode.BAD_REQUEST).json(
                errorResponse(statusCode.BAD_REQUEST, true, "Profile image is required!")
            );
        }
        ...
        return res.status(statusCode.INTERNAL_SERVER_ERROR).json(errorResponse(statusCode.INTERNAL_SERVER_ERROR, true, "Server Error"));
    }
}
...
module.exports.verifyOTP = async (req, res) => {
    try {
        ...
        if (req.body.OTP !== admin.OTP.toString()) {
